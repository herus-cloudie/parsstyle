  ...
  "fixed z-50 gap-4 bg-white p-6 shadow-lg transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-500 dark:bg-slate-950",
  {
    variants: {
      side: {
        ...
        right:
          "inset-y-0 right-0 h-full w-3/4  border-l data-[state=closed]:slide-out-to-right data-[state=open]:slide-in-from-right sm:max-w-[20rem]",
      },
    },
    defaultVariants: {
      side: "right",
...
const SheetHeader = ({
  ...
  <div
    className={cn("flex flex-col items-center space-y-2 text-center sm:text-left", className)}
    {...props} />
)
